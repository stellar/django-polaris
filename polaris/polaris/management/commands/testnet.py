...
class Command(BaseCommand):
    ...
    ``issue`` allows users to create assets on the Stellar testnet network, porting
    the functionality originally offered by the create-stellar-token tool. When the
    test network resets, you’ll have to reissue your assets.

    ``reset`` calls the functionality invoked with issue for each asset in the
    ...
    Updates the paging_token of latest transaction streamed for each anchored asset
        ...
        ``watch_transactions`` streams transactions to and from each anchored asset’s
        distribution account. Specifically, it streams transactions starting with the
        most recently completed transaction’s paging_token on startup. When the
        testnet resets, the paging_token used for transactions prior to the reset are
        no longer valid. To fix this, Polaris updates the paging_token of the most
        recently completed transaction for each anchored asset to "now".
    """
