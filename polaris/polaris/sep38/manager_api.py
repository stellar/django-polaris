...
class OffchainAssetAPIView(APIView):
    ...
    def get(request: Request) -> Response:
        ...
        return Response({"assets": output_assets})
        ...
        for asset in assets:
            ...
            if offchain_asset.schema is None or offchain_asset.identifier is None:
                return render_error_response(
                    description="Missing `schema` or `identifier`",
    ...
    def delete(request: Request) -> Response:
        ...
        if request.data.get("identifier") is not None:
            to_be_deleted = to_be_deleted.filter(
...
def _add_delivery_methods(request: Request, type: str):
    ...
    for dm in dms:
        ...
        if asset_str is None:
            return render_error_response(
                "Missing asset field.", status.HTTP_400_BAD_REQUEST
            )
    ...
    def post(request: Request) -> Response:
        ...
        for offer in offers:
            ...
            if offer.get("buy_delivery_method") is not None:
                exchange_pair.buy_delivery_method = _get_delivery_method(
                    exchange_pair.buy_asset, offer.get("buy_delivery_method"), "buy"
                )
            if offer.get("sell_delivery_method") is not None:
                exchange_pair.sell_delivery_method = _get_delivery_method(
                    exchange_pair.sell_asset, offer.get("sell_delivery_method"), "sell"
                )

            ExchangePair.save(exchange_pair)
            created_offer.append(model_to_dict(exchange_pair))

        return Response({"created": created_offer})
