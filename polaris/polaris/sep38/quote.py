...
def validate_quote_request(r: dict) -> dict:
    ...
    if r.get("expire_after") is not None:
        r["expire_after"] = aniso8601.parse_datetime(r.get("expire_after"))
...
class QuoteAPIView(APIView):
    ...
    def get(request: Request, quote_id: str) -> Response:
        try:
            _quote = Quote.objects.get(id=quote_id)
            return Response(model_to_dict(_quote, fields=quote_response_fields))
    ...
    def post(request: Request) -> Response:
        ...
        eo = ExchangePair.objects.get(sell_asset=quote.buy_asset, buy_asset=quote.sell_asset)
        if eo is None:
